...
const makeURLFriendlyAndCheckIfItsUnique = async (title) => {
    const Url = makeURLFriendly(title)
    const article = await Articles.findOne({frindlyUrl : Url});
    if (article) 
        return null;
    else 
        return Url;
...
const renderArticleContent = async (articleContent) => {
    const markedText = marked.parse(articleContent);
    const html = domPurify.sanitize(markedText);
    return html;
}

module.exports = {
    makeURLFriendlyAndCheckIfItsUnique : makeURLFriendlyAndCheckIfItsUnique,
